...
Seja $(X, \mcal{F}, \mu)$ um espaço de medida. Denotaremos por $\mcal{M}(X, \mcal{F})$ o espaço das funções $X \rightarrow \overline{\mathbb{R}}$ mensuráveis e por $\mcal{M}^+(X, \mcal{F})$ o conjunto das funções em $\mcal{M}(X, \mcal{F})$ que assumem valores não-negativos.
...
$$ \varphi = \sum_{j=1}^n a_j \chi_{E_j}, \quad \text{em que } a_j \in \mathbb{R} \text{ e } E_j \in \mcal{F}. $$
...
\begin{definition*}{}
Seja $\varphi \in \mcal{M}^+(X, \mcal{F})$ uma função simples com representação padrão $\varphi = \sum_{j=1}^n a_j \chi_{E_j}$. Definimos a \textbf{integral de $\varphi$ com respeito a $\mu$} como
...
\begin{prop*}{}
Sejam $\varphi, \psi \in \mcal{M}^+(X, \mcal{F})$ funções simples. Então:
\begin{itemize}
    ...
    \item[d)] A aplicação $\lambda: \mcal{F} \rightarrow \overline{\mathbb{R}}$ dada por $\lambda(E) := \int \varphi \chi_E \, d\mu$ é uma medida em $(X, \mcal{F})$.
\end{itemize}
\end{prop*}
...
\textbf{Exemplo:} Seja $(\mathbb{R}, \mcal{M}, m)$ o espaço de medida de Lebesgue e considere a função de Dirichlet $\varphi: \mathbb{R} \rightarrow \mathbb{R}$ dada por
...
\begin{definition*}{}
Seja $(X, \mcal{F}, \mu)$ um espaço de medida e $f \in \mcal{M}^+(X, \mcal{F})$. Definimos a \textbf{integral de $f$ com respeito a $\mu$} como
$$ \int_X f \, d\mu := \sup \left\{ \int_X \varphi \, d\mu ; \varphi \in \mcal{M}^+(X, \mcal{F}) \text{ é simples e } 0 \le \varphi(x) \le f(x), \forall x \in X \right\} \in \overline{\mathbb{R}}. $$
Para $E \in \mcal{F}$, definimos a integral de $f$ sobre $E$ com respeito a $\mu$ como
$$ \int_E f \, d\mu := \int_X f \chi_E \, d\mu. $$
\end{definition*}

\begin{prop*}{}
Seja $(X, \mcal{F}, \mu)$ um espaço de medida.
\begin{itemize}
    \item[a)] Se $f, g \in \mcal{M}^+(X, \mcal{F})$ e $f \le g$ em $X$, então $\int f \, d\mu \le \int g \, d\mu$.
    \item[b)] Se $E, F \in \mcal{F}$ e $E \subseteq F$, então $\int_E f \, d\mu \le \int_F f \, d\mu$.
\end{itemize}
\end{prop*}
...
Seja $(X, \mcal{F}, \mu)$ um espaço de medida, $f \in \mcal{M}^+(X, \mcal{F})$ e $\lambda > 0$. Considere $E_\lambda = \{x \in X ; f(x) \ge \lambda\}$. Então
$$ \mu(E_\lambda) \le \frac{1}{\lambda} \int f \, d\mu. $$
\end{prop*}

\begin{proof}[\textbf{Dem:}]
Considere a função simples $\lambda \chi_{E_\lambda} \in \mcal{M}^+(X, \mcal{F})$. Note que $\lambda \chi_{E_\lambda}(x) \le f(x)$, $\forall x \in X$. Logo,
...
\begin{corollary*}{}
Seja $(X, \mcal{F}, \mu)$ um espaço de medida e $f \in \mcal{M}^+(X, \mcal{F})$ tal que $\int f \, d\mu < +\infty$. Então $f$ é finita $\mu$-q.t.p. e o conjunto $E = \{x \in X ; f(x) > 0\}$ é $\sigma$-finito.
\end{corollary*}
...
\begin{theorem*}{ da Convergência Monótona}
Seja $(X, \mcal{F}, \mu)$ um espaço de medida e $(f_n)_{n \in \mathbb{N}}$ uma sequência não-decrescente de funções em $\mcal{M}^+(X, \mcal{F})$ que converge pontualmente para $f$, isto é, $f_n(x) \le f_{n+1}(x)$, $\forall n \in \mathbb{N}, x \in X$ e $\lim_{n \to \infty} f_n(x) = f(x)$, $\forall x \in X$. Então
$$ \int \left( \lim_{n \to \infty} f_n \right) d\mu = \int f \, d\mu = \lim_{n \to \infty} \int f_n \, d\mu. $$
\end{theorem*}

\begin{proof}[\textbf{Dem:}]
Como $f$ é o limite pontual de uma sequência em $\mcal{M}^+(X, \mcal{F})$, então $f \in \mcal{M}^+(X, \mcal{F})$. Note que $f_n(x) \le f_{n+1}(x) \le f(x)$, $\forall x \in X$. Assim, pela monotonicidade, temos que
...
Provaremos agora a desigualdade contrária. Seja $\alpha \in (0,1)$ e $\varphi \in \mcal{M}^+(X, \mcal{F})$ uma função simples com $0 \le \varphi \le f$. Considere o conjunto $A_n = \{x \in X ; f_n(x) \ge \alpha \varphi(x)\}$.
...
Como isso vale para qualquer função simples $\varphi \in \mcal{M}^+(X, \mcal{F})$ com $0 \le \varphi \le f$, concluímos pela definição de supremo que $\lim_{n \to \infty} \int f_n \, d\mu$ é uma cota superior para as integrais, e portanto
