...
\begin{proof}[\textbf{Dem:}]
  ...
  \begin{align*}
    ...
    = \sum_{j \in \mbb N} \mu(E_j). 
  \end{align*}
  Por outro lado, a igualdade pode não ser satisfeita quando 
  $\bigcup E_j$ é infinito. Tomando a sequência $\set{E_j}$
  definido por $E_j = \{j\}$ para cada $j \in \bN$, como $\bigcup E_j = \bN$, segue que, 
  \begin{align*}
    \mu\parth{\bigcup E_j} = +\infty \neq 0 = \sum_{j \in \bN} 0 = \sum_{j \in \bN} E_j.
  \end{align*}
  Já que $\mu$ não é $\sigma$-aditiva, temos que $\mu$ não é medida.
\end{proof}
...
\begin{proof}[\textbf{Dem:}]Como a propriedade vale $\mu$-q.t.p, então, existe $N \in \mcal F$ tal que $\mu(N) = 0$ e 
  ...
  \begin{align*}
    \{x \in X \st f(x)> \alpha \} &=
    \{x \in A \st f(x)>\alpha\}\cup \{x \in A^C \st f(x) > \alpha\}\\
    &= \{x \in A \st f(x)>\alpha\}\cup \{x \in A^C \st \lim \inf f_n(x) > \alpha\}\\
    &= \{x \in A \st f(x)>\alpha\}\cup \cparth{A^C \cap \{x \in X \st 
    \lim \inf f_n(x) > \alpha\}}. 
  \end{align*}
  Visto que $A \in \mcal F$, temos $A^C \in \mcal F$. Mais ainda, já que $(f_n)$ é 
  uma sequência de funções mensuráveis, segue que $\lim \inf f_n$ também é mensurável,
  portanto,
  $\{x \in X; \lim \inf f_n(x)> \alpha\} \in \mcal F$, 
