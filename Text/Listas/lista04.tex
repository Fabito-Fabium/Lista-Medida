...
\begin{proof}[\textbf{Dem:}].\\
  ...
  \begin{align*}
    2^{p+1} \pnorm p f^p&= \int 2^{p+1}\mparth{f}^p d\mu =\int \lim \inf g_n d\mu \le \lim \inf \int g_n d\mu \\ 
    &= \lim \inf \int 2^p(\mparth{f_n}^p + \mparth{f}^p) - \mparth{f_n - f}^p d\mu\\
    &= \lim \inf 2^p \cparth{\int \mparth{f_n}^p d\mu + \int \mparth{f} d\mu} - \int \mparth{f_n - f}^p d\mu\\
    &= 2^{p + 1} \pnorm p f^p - \lim\inf \pnorm p {f_n - f}^p. 
  \end{align*}
  Como $f \in L^p(X, \mu)$, 
  \begin{align*}
    2^{p+1}\pnorm p f^p \le 2^{p+1}\pnorm p f^p - \lim \inf \pnorm p {f_n - f}^p \implies
    \lim \inf \pnorm p {f_n - f}^p \le 0.
  \end{align*}
  Como $\pnorm p {f_n - f}^p \ge 0$ para todo $n \in \bN$, segue que $\lim \inf \pnorm p {f_n - f}^p = 0$.
\end{proof}
